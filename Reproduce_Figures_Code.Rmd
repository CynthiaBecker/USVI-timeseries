...
benthicszn <- ggplot(benthic_meta_richness, aes(x = wetdryseason, y = estimate)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(position = position_jitter(width=.1, height=0), aes(color = wetdryseason), size = 3) +
  labs(y = "Breakaway estimate of richness", x = "Season", color = "Season") +
  theme(panel.grid.minor = element_blank(), 
        panel.background = element_blank()) 
...
# Put the supplementary alpha diversity season graphs together (will go with a beta diversity graph)
ggarrange(benthicszn, surfaceszn, common.legend = TRUE, labels = c("b.", "c."))
```
...
# Since it is not significantly different by depth, proceed by combining these together for the analyses:
# plot PCA showing reef and disturbance and disturbance nested within reef
plot_ordination(ps.beta_clr, ps.beta_clr_euc, type="samples", color= "disturbance") +
  coord_fixed() +
  scale_color_brewer(palette = "Dark2") +
  geom_point(size = 3, alpha = 0.7) +
  labs(color = "Disturbance", title = "Figure 5a. Beta diversity in benthic and surface reef water", ) +
...
plot_ordination(ps.beta_clr, ps.beta_clr_euc, type="samples", color="wetdryseason", shape = "disturbance") +
  coord_fixed() +
  geom_point(size = 3, alpha = 0.7) +
  labs(color = "Wet or Dry Season", shape = "Disturbance", title = "PCA on surface and benthic microbial community") +
  theme(panel.grid.minor = element_blank(), 
        panel.background = element_blank())

plot_ordination(ps.beta_clr, ps.beta_clr_euc, type="samples", color = "yearmonth") +
  coord_fixed() +
  geom_point(size = 3, alpha =  0.7) +
  scale_color_viridis_d() + 
  labs(color = "Sampling Timepoin", title = "Seawater microbial community colored by sampling timepoint") +
  theme(panel.grid.minor = element_blank(), 
        panel.background = element_blank())
...
#Could add in a variance partitioning analysis that investigates the variance explained by disturbance, reef, and time period (maybe season?)
