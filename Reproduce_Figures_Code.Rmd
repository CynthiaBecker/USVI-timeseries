...
Note that using filtered data for alpha diveristy can lead to incorrect estimations. Especially since I am interested in estimating richness. To circumvent this, I will use the full dataset ~33k ASVs that have not been filtered to remove low-abundance ASVs. The "filt" extension in this case only refers to the fact that these tables have been filtered of DNA extraction contaminatns (via decontam) and filtered of a few samples that had abnormally low sequencing depth. 
...
Also get coral cover at the start and end of the study
```{r}
...
# How much coral cover was there at the first and last time period?
coralstart_end <- benthic.forFigures %>% 
  tidyr::spread(key = datatype, value = value) %>%
  mutate(totalcoral = (HardCoral + SoftCoral)*100) %>%
  dplyr::select(site, yearmonth, totalcoral) %>%
  filter(yearmonth %in% c("2016-06", "2022-06"))
```
...
# How much algal cover was there at the first and last time period?
algaestart_end <- benthic.forFigures %>% 
  tidyr::spread(key = datatype, value = value) %>%
  mutate(totalalgae = (Ramicrusta + Macroalgae + TurfAlgae)*100) %>%
  dplyr::select(site, yearmonth, totalalgae) %>%
  filter(yearmonth %in% c("2016-06", "2022-06"))
```
...
ggplot(benthic.forFigures, aes(x = Date, y = value, color = site)) +
  ...
  labs(y = "Relative cover", x = "Date", color = "Reef", shape = "Disturbance", title = "Benthic organism cover over time")

#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/Benthic_cover_over_time.pdf", width = 10, height = 6)
...
ggplot(nuts.forFigures, aes(x = Date, y = value, color = site)) +
  ...
  ggtitle("Nutrients over time")

#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/Nuts_over_time.pdf", width = 12, height = 6)
...
ggplot(cells.forFigures, aes(x = Date, y = value, color = site)) +
  ...
  scale_color_carto_d(palette = "Safe") +
  facet_wrap( ~ datatype, scales = "free_y", ncol = 2) +
  labs(y = "Abundance (cells per milliliter)", shape = "Depth", color = "Reef", title = "Cell abundances over time") 

#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/Cells_over_time.pdf", width = 7, height = 6)
```

# REVISIONS Figures and Analyses for paper revision
...
vardata <- filter(env.data.foranalysis, datatype == "po4_um")
...
```
## What about prochloroccocus versus ammonium concentration to go along with the sequence data results?
...
ggplot(pronh4, aes(x = nh4_um, y = pro_per_ml)) +
  ...
  ylim(0,150000) +
  scale_color_brewer(palette = "Dark2") +
  labs(x = "Ammonium (uM)", y = "Prochlorococcus abundance (cells ml-1)", color = "Disturbance")

lm <- lm(pro_per_ml ~ nh4_um, data = pronh4)
summary(lm)

#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/SuppFig_pro_vs_nh4.pdf", width = 5, height = 3)
```
...
dist
depth <- adonis2(allmatrix_vars_noNA ~ depthtype, data = collinear_remove, method = "eu")
depth
...
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/SuppFig_Corncob_all_NH4.pdf", height = 5, width = 8)
```
...
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/SuppFig_RandomForest_MSE_top50.pdf", height = 8, width = 12)
```
...
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/Fig5_DA_RF_shared_NH4_10.26.23.pdf", width = 9, height = 3.5)
```
...
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/Fig6_DA_RF_relabund_regression.pdf", width = 12, height = 6)
```


# REVISED alpha diversity

# Alpha and beta diversity analyses

## Alpha diversity

Make sure to use the `ps.a` physloseq object since that one has all the low abundaance ASVs RETAINED. This is important since I am trying to estimate the true richness, so I cant filter out half of the data set. 
...
alphadepth
```
Based on this, alpha diversity significantly changes with depth, therefore, I will split up the alpha diversity analyses. THIS IS VERY SIMILAR TO THE PREVIOUS GRAPH! BUT BEFORE I MADE AN ERROR AND IDDN'T USE THE UNFILTERED DATA

### Benthic - Does disturbance, reef, and sampling timepoint influence alpha diversity? Also test for season.
```{r alpha diversity benthic}
## BENTHIC ANALYSIS FIRST #####
ps.a_benthic <- ps.a %>%
  subset_samples(site != "Sand patch") %>%  # get rid of sandpatch
...
bt_yearmonth_fixed <- betta(formula = estimate ~ yearmonth, 
                      ses = error, data = benthic_meta_richness)
bt_yearmonth_fixed$table
bt_yearmonth_fixed$global[2] # get global p-value to see if there are changes across all reefs

# I hypothesize that alpha diversity cahnges with disturbance - IT DOES
bt_disturbance_fixed <- betta(formula = estimate ~ disturbance, 
                      ses = error, data = benthic_meta_richness)
bt_disturbance_fixed$table
bt_disturbance_fixed$global[2] # get global p-value to see if there are overall changes across disturbance

# I hypothesize that season (wet vs. dry) will have a significant impact on alpha diversity - IT DOES NOT
bt_season_fixed <- betta(formula = estimate ~ wetdryseason, 
                      ses = error, data = benthic_meta_richness)
bt_season_fixed$table
...
# Check if timepoint is significant within each reef grouping - AKA, does time influence alpha diversity when accounting for site-based differences?
bt_yearmonth_fixed_reef_random <- betta_random(chats = benthic_meta_richness$estimate,
                                       ses = benthic_meta_richness$error,
                                       X = model.matrix(~ yearmonth, data = benthic_meta_richness),
                                       groups = benthic_meta_richness$site)
bt_yearmonth_fixed_reef_random$table
bt_yearmonth_fixed_reef_random$global[2]
...
benthicszn <- ggplot(benthic_meta_richness, aes(x = wetdryseason, y = estimate)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(position = position_jitter(width=.1, height=0), aes(color = wetdryseason, shape = disturbance), size = 3, alpha = 0.7) +
  labs(y = "Breakaway estimate of richness", x = "Season", color = "Season", shape = "Disturbance", title = "Benthic richness") +
  theme(panel.grid.minor = element_blank(), 
        panel.background = element_blank()) 
...
# Check if timepoint is significant within each reef grouping - AKA, does time influence alpha diversity when accounting for site-based differences?
bt_yearmonth_fixed_reef_random <- betta_random(chats = surface_meta_richness$estimate,
                                       ses = surface_meta_richness$error,
                                       X = model.matrix(~ yearmonth, data = surface_meta_richness),
                                       groups = surface_meta_richness$site)
bt_yearmonth_fixed_reef_random$table
bt_yearmonth_fixed_reef_random$global[2]
...
# REVISED Beta Diversity
## Beta diversity

### Does depth influence beta diversity?
Make sure to use the `ps` phyloseq object that has been low abundance filtered so it has about 14,000 taxa
...
### Does disturbance, reef, time period, and season influence beta diversity?
```{r beta diversity disturbance reef}
...
betareef <- plot_ordination(ps.beta_clr, ps.beta_clr_euc, type="samples", color = "disturbance") +
  ...
  geom_point(size = 3, alpha =  0.7) +
  scale_color_brewer(palette = "Dark2") +
  labs(color = "Disturbance", title = "PCA on seawater microbial community faceted by reef") +
  theme(panel.grid.minor = element_blank(), 
        panel.background = element_blank())

betaszn <- plot_ordination(ps.beta_clr, ps.beta_clr_euc, type="samples", color="wetdryseason", shape = "disturbance") +
  coord_fixed() +
  geom_point(size = 3, alpha = 0.7) +
  labs(color = "Wet or Dry Season", shape = "Disturbance", title = "Benthic & surface microbial beta diversity") +
  theme(panel.grid.minor = element_blank(), 
        panel.background = element_blank())
...
betamain <- plot_ordination(ps.beta_clr, ps.beta_clr_euc, type="samples", color = "yearmonth", shape = "disturbance") +
  ...
  labs(color = "Sampling Timepoin", title = "Beta diversity in benthic and surface reef seawater") +
  theme(panel.grid.minor = element_blank(), 
        panel.background = element_blank(), 
        legend.position = "bottom")
...
adonis2(formula = euc_diss ~ disturbance, data = clr_meta, permutations = 999) 
adonis2(formula = euc_diss ~ site/disturbance, data = clr_meta, permutations = 999) #disturbance nested within site
adonis2(formula = euc_diss ~ wetdryseason, data = clr_meta, permutations = 999)
adonis2(formula = euc_diss ~ yearmonth, data = clr_meta, permutations = 999)

#Note: I tried variance partitioning, which employs an RDA analysis to investigate to what degree these different variables contribute to changes in microbial community composition. However, because some of the variables are collinear (makes sense since disturbance, timepoint, and season are all interwoven) I cannot fully trust the results. 
```

# REVISED DIVERSITY FIGS
```{r}
...
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/SuppFig_AlphaDiv.pdf", width = 9, height = 9)
```
...
ggarrange(betamain, 
          ...
          ncol = 2, labels = c("a.", "b."))
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/Fig4_Diversity.pdf", width = 9, height = 5)

# Supplementary Figure SXXX
# Microbial beta diversity, alpha diversity by season
ggarrange(betaszn, benthicszn, surfaceszn, 
          ncol = 3, labels = c("a.", "b.", "c."), common.legend = TRUE, widths = c(2,1,1), legend = "bottom")
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/SuppFig_Season_Diversity.pdf", width = 9, height = 5)

# Supplementary Figure S8
# Microbial alpha and beta diversity across disturbbances within individual reefs
ggarrange(betareef, 
          common.legend = TRUE, legend = "bottom")
#ggsave("/Users/cynthiabecker/Documents/Apprill_lab/USVI_Projects/Publication_Figures/EMI_Revision_Figures/SuppFig_Reef_Beta_diversity.pdf", width = 7, height = 5)
```
